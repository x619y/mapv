	...
	var Layer$6 = function (_BaseLayer) {
		...
		createClass(Layer, [{
			...
			value: function _canvasUpdate(time) {
				...
				// var scale = Math.pow(2, map.getView().getZoom()) * map.getView().getResolution();
				// var offset = mapProjection.fromLatLngToPoint(this.canvasLayer.getTopLeft());
				var dataGetOptions = {
					//fromColumn: self.options.coordType == 'bd09mc' ? 'coordinates' : 'coordinates_mercator',
					transferCoordinate: function transferCoordinate(coordinate) {
						if(!ol) return [0,0];
						var coorsArr = ol.proj.transform([coordinate[0],coordinate[1]],'EPSG:900913','EPSG:4326');
						return map.getPixelFromCoordinate(coorsArr);
					}
				};
				...
				var  pixelArr = map.getPixelFromCoordinate([0,0]);
				var pixel = {
					x: pixelArr[0],
					y: pixelArr[1]
				};
